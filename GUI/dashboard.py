...
class Dashboard(QWidget):
    def __init__(self):
        ...
        grid_layout.addWidget(self.create_indicator("75", "CRITICAL", "#FF4B3E"), 0, 0)
        grid_layout.addWidget(self.create_indicator("849", "HIGH", "#F9C846"), 0, 1)
        grid_layout.addWidget(self.create_indicator("384", "MEDIUM", "#FF964F"), 0, 2)
        grid_layout.addWidget(self.create_indicator("31", "LOW", "#9DD74D"), 0, 3)
        grid_layout.addWidget(self.create_indicator("225", "INFO", "#3199FF"), 0, 4)
        pie_chart_view = self.create_pie_chart()
        pie_chart_view.setFixedSize(400, 300)  
        grid_layout.addWidget(pie_chart_view, 1, 5, 1, 1)  

        line_chart_view = self.create_line_chart()
        line_chart_view.setFixedSize(400, 300)  
        grid_layout.addWidget(line_chart_view, 2, 5, 1, 1)  
        self.setStyleSheet("QWidget{background-color:#FFFFFF}");

        white = QWidget()
        white.setStyleSheet("background-color:white")
        grid_layout.addWidget(white,0,5,1,1)
